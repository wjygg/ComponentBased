...
/**
 * Created by wangjingyun on 2017/5/11.
 */

public class ExctptionCarshHandler implements Thread.UncaughtExceptionHandler{

    private static ExctptionCarshHandler mInstance;

    private Context context;

    private Thread.UncaughtExceptionHandler defaultUncaughtExceptionHandler;

    public static ExctptionCarshHandler getInstance(){

        if(mInstance==null){

            synchronized (ExctptionCarshHandler.class){

                if(mInstance==null){

                    mInstance=new ExctptionCarshHandler();
    ...
    public File getCrashFile() {
        String crashFileName = context.getSharedPreferences("crash",
                Context.MODE_PRIVATE).getString("CRASH_FILE_NAME", "");

        if(crashFileName!=null&&!"".equals(crashFileName)){
